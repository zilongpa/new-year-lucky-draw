...
function Slot({ names }: { names: string[] }) {
 ...
 const initialY = -(useMemo<number>(() => { return animationCurrentIndex }, []) * itemHeight);
      ...
      >
        ...
        >
          {animationParticipants.map((name, idx) => (
            ...
            >
              <h2 style={{textAlign: "center", height: "5vw", fontSize: "4vw"}}>{name}</h2>
            </div>
          ))}
        </motion.div>
      </div>
  ...
  return (
    <>
      <main style={{ backgroundImage: `url(${background})` }}>
        <h1 style={{ position: "absolute", fontSize: "4.5vw", top: "22%" }}>{title}</h1>
